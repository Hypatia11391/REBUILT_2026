...
public class AimInstance {
    ...
    private final Pose3d targetPositionRed = new Pose3d(
        4.611624,
        4.034536,
        1.8288,
        defaultRotation
    );
    private final Pose3d targetPositionBlue = new Pose3d(
        11.901424,
        4.034536,
        1.8288,
        defaultRotation
    ...
    public void updateAim() {
        ...
        timeGuess = newtonsMethodFunc(
            distanceToTarget,
            robotVelocities,
            timeGuess,
            height,
            cotAlpha,
            newtonsMethodIterations
        );

        double effectiveDistanceX =
            distanceToTarget.getX() -
            robotVelocities.vxMetersPerSecond * timeGuess;
        double effectiveDistanceY =
            distanceToTarget.getY() -
            robotVelocities.vyMetersPerSecond * timeGuess;

        rotateBy = Math.atan2(effectiveDistanceY, effectiveDistanceX);
        exitVelocity =
            (height + 0.5 * gravity * Math.pow(timeGuess, 2)) /
            (timeGuess * Math.sin(shooterAngle));
    }

    // Newtons method is an iterative function that calculates the time that an object will be in the air.
    private double newtonsMethodFunc(
        Translation3d distanceToTarget,
        ChassisSpeeds robotVelocities,
        double time,
        double height,
        ...
        for (int i = 0; i < repetitions; i++) {
            double dx =
                distanceToTarget.getX() -
                robotVelocities.vxMetersPerSecond * time;
            double dy =
                distanceToTarget.getY() -
                robotVelocities.vyMetersPerSecond * time;
            ...
            double func =
                cotAlpha * (height + (gravity / 2) * time * time) -
                effectiveDistanceMagnitude;
            double funcPrime =
                cotAlpha * gravity * time -
                (robotVelocities.vxMetersPerSecond * HATeffectiveDistanceX +
                    robotVelocities.vyMetersPerSecond * HATeffectiveDistanceY);

            if (
                effectiveDistanceMagnitude < 1e-6 || Math.abs(funcPrime) < 1e-9
            ) break; // safegaurd against div by 0
    ...
    public void updateRobotState(DriveBase driveBase) {
        shooterPosition = new Pose3d(
            DriveBase.getPose2D().getX(),
            DriveBase.getPose2D().getY(),
            shooterSetHeight,
            defaultRotation
        );
    ...
    public boolean isAutomaticAimControl() {
